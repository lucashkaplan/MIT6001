from ps3_test_play_hand import *
...
def test_is_valid_word(word_list):
    ...
    # test 8
    # word "evil" should continue to be valid even if played multiple times
    hand = {'e':1, 'v':2, 'n':1, 'i':1, 'l':2}
    word = "EVIL"
    ...
    if  not is_valid_word(word, hand, word_list):
        print("FAILURE: test_is_valid_word()")
        print("\tExpected True, but got False for second time using word: '" + word + "' and hand:", hand)
        
        failure = True
    
    # test 9
    # check if illegal word "phone" flagged, b/c 'p' doesn't exist in hand
    word = "phone"
...
def test_wildcard(word_list):
    ...
    # test 5
    hand = {'d': 1, 'o': 2, '*': 2, 'w': 1, 'r':1, 'd':1, 'y': 2}
    word = "w**rd"
